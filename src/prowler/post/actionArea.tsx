    ...
    return <div className="flex flex-row flex-nowrap">
        ...
        { menuEnabled && <><div className="bg-(--mid-pri) box-glow-[black] select-none rounded-lg absolute z-7 min-w-36">
            ...
            <button className="w-full flex items-center rounded-lg bg-transparent hover:bg-(--sec) active:bg-(--tri) p-0 opacity-100"
                ...
                <span className="pr-3 text-(--main) text-[1rem] block leading-none">Edit</span>
            </button>
            <button className="w-full flex items-center rounded-lg bg-transparent hover:bg-(--sec) active:bg-(--tri) p-0 opacity-100"
                    title="Delete" onClick={() => setDeleteDialogEnabled(true)}>
                <svg className="box-content relative fill-(--main) p-2 w-5 h-5 block shrink-0" viewBox="0 0 48 48"
                     xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="pr-3 text-(--main) text-[1rem] block leading-none">Delete</span>
            </button><hr /></> }
            <button className="w-full flex items-center rounded-lg bg-transparent hover:bg-(--sec) active:bg-(--tri) p-0 opacity-100"
                    title="Call Dr. Eason" onClick={(e) => {
                ...
                <span className="pr-3 text-(--main) text-[1rem] block leading-none">Call Dr. Eason</span>
            </button>
            <button className="w-full flex items-center rounded-lg bg-transparent hover:bg-(--sec) active:bg-(--tri) p-0 opacity-100"
                    title="Report" onClick={(e) => {
                ...
                <span className="pr-3 text-(--main) text-[1rem] block leading-none">Report</span>
            </button>
...
const CloseArea = ({ setActive }: { setActive: (active: boolean ) => void }) => {
    return <div className="fixed inset-0 z-5" onClick={() => setActive(false)} />
};