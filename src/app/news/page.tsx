...
async function fetchMaxPrepsSchedule(url: string): Promise<Array<{date?: string; name?: string; location?: string; result?: string; link?: string}>> {
  try {
    const res = await fetch(url, { next: { revalidate: 3600 } })
    if (!res.ok) return []
    const html = await res.text()
    ...
    for (const ev of events) {
      ...
      if (ev.event && Array.isArray(ev.event)) {
        ev.event.forEach((e: any) => out.push({date: e.startDate, name: e.name, location: e.location && e.location.name, link: e.url || url}))
      }
      if (ev.events && Array.isArray(ev.events)) {
        ev.events.forEach((e: any) => out.push({date: e.startDate, name: e.name, location: e.location && e.location.name, link: e.url || url}))
      }
    }
...
export default async function Page() {
  ...
  return (
    <main style={{padding: 20}}>
      <h1>School News</h1>
      {!latest && <p>No videos found.</p>}
      {latest && (
        <article key={latest.id} style={{border: '1px solid #ddd', padding: 12, borderRadius: 6}}>
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8}}>
            <strong>{latest.title}</strong>
            {isNew(latest.published) && (
              <span style={{background: '#e11', color: 'white', padding: '4px 8px', borderRadius: 4}}>New</span>
            )}
          </div>
          <div style={{position: 'relative', paddingTop: '56.25%'}}>
            <a href={`https://www.youtube.com/watch?v=${latest.id}`} target="_blank" rel="noopener noreferrer" style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}>
              <img
            ...
            {teamSchedule && teamSchedule.length > 0 ? (
              <ul style={{paddingLeft: 16, margin: 0}}>
                {teamSchedule.map((s, i) => (
                  <li key={`team-${i}`} style={{marginBottom: 6}}>
                    <a href={s.link || MAXPREPS_TEAM_URL} target="_blank" rel="noopener noreferrer">{s.date ? `${s.date} — ` : ''}{s.name || 'Game'}</a>
                    {s.location ? <span style={{color: '#666'}}> — {s.location}</span> : null}
                  </li>
                ))}
              </ul>
            ) : teamOg ? (
              <a href={MAXPREPS_TEAM_URL} target="_blank" rel="noopener noreferrer">
                <img src={teamOg} alt="MaxPreps team" style={{width: '100%', height: 'auto', display: 'block'}} />
              </a>
            ) : (
              <a href={MAXPREPS_TEAM_URL} target="_blank" rel="noopener noreferrer" style={{display: 'block', padding: 24, textAlign: 'center', color: '#666'}}>Open Team Page</a>
            )}
          </div>
          <div style={{border: '1px solid #ddd', padding: 12}}>
            ...
            {scheduleList && scheduleList.length > 0 ? (
              <ul style={{paddingLeft: 16, margin: 0}}>
                {scheduleList.map((s, i) => (
                  <li key={`sched-${i}`} style={{marginBottom: 6}}>
                    <a href={s.link || MAXPREPS_SCHEDULE_URL} target="_blank" rel="noopener noreferrer">{s.date ? `${s.date} — ` : ''}{s.name || 'Game'}</a>
                    {s.result ? <strong style={{marginLeft: 8}}>{s.result}</strong> : null}
                  </li>
                ))}
              </ul>
            ) : scheduleOg ? (
              <a href={MAXPREPS_SCHEDULE_URL} target="_blank" rel="noopener noreferrer">
                <img src={scheduleOg} alt="MaxPreps schedule" style={{width: '100%', height: 'auto', display: 'block'}} />
              </a>
            ) : (
              <a href={MAXPREPS_SCHEDULE_URL} target="_blank" rel="noopener noreferrer" style={{display: 'block', padding: 24, textAlign: 'center', color: '#666'}}>Open Schedule</a>
            )}
          </div>
        </div>
      </section>
        ...
        <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start'}}>
          <div>
            ...
            <ul style={{paddingLeft: 16}}>
              <li><a href="https://gofan.co/app/school/FL555" target="_blank" rel="noopener noreferrer">School Ticket Portal (GoFan)</a></li>
            </ul>
          </div>
          <div>
            ...
            <ul style={{paddingLeft: 16}}>
              <li><a href="https://www.cait.scps.k12.fl.us/forms-clearance" target="_blank" rel="noopener noreferrer">Athletic Clearance</a></li>
              <li><a href="https://www.paypal.com/donate/?hosted_button_id=EY6KCVGMKDBNC" target="_blank" rel="noopener noreferrer">Donate</a></li>
            </ul>
          </div>
        </div>
      </footer>
