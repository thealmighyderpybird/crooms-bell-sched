...
export default function Announcement({ announcement }: { announcement: Announcement }) {
    ...
    return <div className={ styles.accordion }>
        <div className={ styles.accordionHeader } onClick={() => setShowAccordion(!showAccordion)}>
            { announcement.data.title }</div>
        { showAccordion && <div className={ styles.accordionContent }>
