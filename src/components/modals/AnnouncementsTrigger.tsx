...
export default function AnnouncementsTrigger() {
    ...
    useEffect(() => {
        const handler = (e: Event) => {
            try {
                // event detail contains ids
                // @ts-ignore
                const ids = e?.detail ?? JSON.parse(localStorage.getItem('viewed-announcements') ?? '[]');
                setViewedAnnouncements(ids);
            } catch (err) { }
        };
        window.addEventListener('announcements-read-all', handler as EventListener);
        return () => window.removeEventListener('announcements-read-all', handler as EventListener);
    }, []);
